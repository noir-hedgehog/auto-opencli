...
function groupRows(rows) {
    ...
    for (const row of rows) {
        ...
        byAssignee.set(row.assignee || '-', (byAssignee.get(row.assignee || '-') || 0) + 1);
    }
...
cli({
    ...
    columns: ['view', 'total', 'visibleRows', 'statusBreakdown', 'assigneeBreakdown', 'latestUpdateTime'],
    func: async (page, args) => {
        ...
        for (const view of views) {
            ...
            const grouped = groupRows(mapped);
            ...
            rows.push({
                ...
                statusBreakdown: topEntries(grouped.byStatus),
                assigneeBreakdown: topEntries(grouped.byAssignee),
                latestUpdateTime,
            });
        }
        return rows;
